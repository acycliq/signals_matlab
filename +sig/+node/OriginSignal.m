classdef OriginSignal < sig.node.Signal
  ...
  methods
    ...
    function post2(this, value)
        % The whole transaction (propagate then apply, what the MEX did
        % through the submit + applyNodes pair) lives on the node itself,
        % see sig.node.Node/transact. Net.runSchedule delivers delayed
        % posts through the same method.
        this.Node.transact(value);
    end
